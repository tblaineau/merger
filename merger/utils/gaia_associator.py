...
j=0
for pier in ["W", "E"]:
	for chunks in quartpc:#pc:
		print(" ")
		print(pier, chunks)
		c_macho = macho[(macho["pier"]==pier) & (macho["chunk"].isin(chunks))]
		if len(c_macho)==0:
			continue
		c_temp_macho = c_macho.iloc[np.random.randint(0, len(c_macho), 5000)]
		if chunks == [255]:
		...
		prec = (d2d.arcsec < 0.2).sum() / (d2d.arcsec < 2).sum()
		while i < imax:
			print(i)
